...
import { NtgAgent } from "./_ntgAgent";
...
async function handleCopilotRequest(req: NextRequest, integrationId: string) {
  ...
  try {
    ...
    // Build the agent here so we have the live request cookies.
    // Pass a plain Record<string, AbstractAgent> — NOT a factory function —
    // because CopilotRuntime.handleServiceAdapter() resolves the agents field with
    // Promise.resolve(), which wraps a function as-is and yields an empty key set.
    const cookie = req.headers.get("cookie") ?? "";
    const agentInstance = new NtgAgent({ agentId: integrationId, cookieHeader: cookie });
    console.log(`[Copilot Handler] Initialized NtgAgent for ID: ${integrationId}`);

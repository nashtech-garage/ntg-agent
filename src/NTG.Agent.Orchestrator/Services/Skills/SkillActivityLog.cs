...
/// rendering) during a chat run. Because the buffer is scoped, both the agent and any inner
/// agents it delegates to (created within the same request) write to the same instance — so activity
/// deep inside an sub-agent still surfaces to the UI. <see cref="Agents.AgentService"/> drains it

...
public class AgentService
{
    ...
    private async IAsyncEnumerable<string> InvokePromptStreamingInternalAsync(string message, List<ChatMessage>? previousMessages)
    {
        ...
        var prompt = $@"
           ...
           If the answer is empty, continue to answer with your knowledge and tools or plugins. Otherwise reply with the answer and include citations to the relevant information where it is referenced in the response";
        ...
        {
            Name = "NTG-Assistant",
            Instructions = @"Your are an NGT AI Assistant. Awnser questions with all your best.",
            Kernel = agentKernel,
            Arguments = arguments,
        };

...
public partial class TokenUsageController : ControllerBase
{
    ...
    /// <param name="fromDate">Start date for filtering (UTC)</param>
    /// <param name="toDate">End date for filtering (UTC)</param>
    ...
    {
        var stats = await _tokenTrackingService.GetUsageStatsAsync(userId, sessionId, fromDate, toDate);
        return Ok(stats);
    }
        ...
        [FromQuery] int pageSize = 50)
    {
        ...
        var result = await _tokenTrackingService.GetUsageHistoryAsync(fromDate, toDate, page, pageSize);
        ...
        [FromQuery] int topN = 0)
    {
        ...
        var stats = await _tokenTrackingService.GetStatsByUserAsync(fromDate, toDate, topN);
        return Ok(stats);
    }
}

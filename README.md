...
> **Note on databases:** if you use your own SQL Server instead of the AppHost's `sqlserver` container, update the connection string in the appsettings.Development.json files of NTG.Agent.Admin, NTG.Agent.Orchestrator, and NTG.Agent.MCP.Server (the MCP server uses its own `NTGAgentSkills` database for Agent Skills), then run `dotnet ef database update` in each of those three projects.
